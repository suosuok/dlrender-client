...
class RenderShot(object):
    ...
    def shot_frame_buffer(self):
        ...
        shot_ext2 = os.path.join(self.shot_dst, "max_shotwindow.exe")
        shot_out = self.get_shot_outfile()
        print("shot_out = {}".format(shot_out))
        ...
        cmds2 = '"{0}" -f "{1}"'.format(shot_ext2, shot_out.encode(sys.getfilesystemencoding()))
        try:
            
            self.logger("Start to render shot window")
            self.logger("CMDS2 = {}".format( cmds2))
            self.logger("self.shot_dest2 = {}".format( self.shot_dst))
            subprocess.check_output( cmds2, stderr=subprocess.PIPE, cwd=self.shot_dst, shell=True)
        except subprocess.CalledProcessError as e:
            print(e)
