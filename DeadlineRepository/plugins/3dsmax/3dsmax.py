        ...
        if self.IsDBRJob:
            ...
            if self.GetCurrentTaskId() == "0":
                ...
                if self.VrayDBRJob:
                    ...
                    self.LogInfo( self.Prefix + "Auto-Start Local Worker: %s" % self.MyMaxController.VrayRtDBRAutoStartLocalSlave )
                elif self.MentalRayDBRJob:
...
class MaxController( object ):
    ...
    def Initialize( self ):
        ...
        if SubmittedFromVersion != "":
            ...
            if SlaveExeVersion != SubmittedFromVersion:
                self.Plugin.LogWarning( "Worker's 3ds Max version is NOT the same as the 3ds Max version that was used to submit this job! Unexpected results may occur!" )
    ...
    def RenderTasks( self ):
        ...
        if not self.Plugin.GetBooleanPluginInfoEntryWithDefault( "SkipRender", False ):
            ...
            while frame <= self.EndFrame:
                shot_obj = RenderShot(job=self.Plugin.GetJob(), task_id=self.Plugin.GetCurrentTaskId(), frame=frame, logger=self.Plugin.LogInfo)
                shot_obj.start_shot()
    ...
    def CreateStartupScript( self, port ):
        ...
        if len(selectedGPUs) > 0:
            ...
            self.Plugin.LogInfo( "This Worker is overriding its GPU affinity, so the following GPUs will be used by Octane/RedShift/V-Ray RT: %s" % gpus )
...
class VRaySpawnerProcess( ManagedProcess ):
    ...
    def InitializeProcess( self ):
        ...
        if self.IsRt:
            ...
            if len(selectedGPUs) > 0:
                vrayGpus = "index" + ";index".join( [ str( gpu ) for gpu in selectedGPUs ] ) # "index0;index1"
                self.Plugin.LogInfo( "This Worker is overriding its GPU affinity, so the following GPUs will be used by V-Ray RT: %s" % vrayGpus )
    ...
    def RenderExecutable( self ):
        ...
        if self.IsRt:
            ...
            if int(version) > 2021:
                vraySpawnerExecutable = r"C:\ProgramData\Autodesk\ApplicationPlugins\VRay3dsMax{}\bin\vrayspawner2022.exe".format(
                    version)
            else:
